...
1. **Foundation** — schema + migrations, Supabase Auth + RLS, admin panel skeleton with MFA, mock providers wired end-to-end. **Admin panel skeleton built and browser-tested (2026-08-28)**: `admin_users`-gated login, TOTP MFA enrollment + challenge (both actually exercised end-to-end in a browser, not just code review), businesses list/detail/create. **Owner-account creation/invite flow built and verified (2026-08-29)** — see known-blocker #10, now resolved. Foundation phase is complete.
   Hardening layers on top of `admin_users`+MFA gating (defense in depth, never the security boundary itself): host-aware `app/robots.ts` (disallows `/admin` and `/app` today, will disallow everything once `admin.`/`app.` are real separate hosts); no links to `/admin` from marketing pages ever; the equivalent "no links to `/app`" rule was reversed 2026-08-30 (ADR-0024) once real owner accounts existed and needed a discoverable way back in — a single "Log in" link in the marketing header pointing to `/app/login` is the only exception, and it doesn't change the actual security boundary (RLS + auth), just discoverability; per-IP and per-(IP+email) rate limiting on both `/api/admin/login` and `/api/app/login` (`lib/rate-limit/`, in-memory locally, swaps to Upstash automatically once `UPSTASH_REDIS_REST_URL`/`TOKEN` are set — actually verified tripping correctly, both layers, not just written).
2. **Shared engine** — pipeline engine, template/auto-reply matching w/ priority+conflict rules, reminder engine (`pg_cron`+`pg_net`, `FOR UPDATE SKIP LOCKED`), webhook durability pattern, kill switch, manual recovery tools. **Added 2026-09-02 (ADR-0030, confirmed pre-friend-testing bug fixes)**: the webhook durability pattern's own promised recovery job now exists — `claim_stuck_webhook_event()` + `recoverStuckWebhookEvents()` (`lib/engine/webhook-durability.ts`), mirroring `claim_next_reminder()`/`recover_stuck_reminders()` exactly, run from the same `/api/cron/reminders` tick rather than a second schedule. Also fixed: the manual "Send Reminder" one-per-day idempotency key computed "today" from the server's UTC clock instead of the business's own IST calendar day (`getBusinessDateString()`, `lib/engine/business-day.ts`) — a real, deterministic bug for the ~5.5-hour daily window where UTC and IST disagree on the date.
2.5. **Owner app (not in the original 6-phase plan — inserted 2026-08-29)**: a real business-owner-facing UI, built against Phase 2's shared engine + mock providers. Per ADR-0020, every screen and workflow here must be built and verified against mocks before Phase 6 (real provider integration) begins at all — six screens exist now (Today, Customers List, Contact Detail, Needs Attention, Payments, Settings — labeled "Customers" in the UI since ADR-0027; the route stays `/app/contacts`, unrenamed, since that's an internal path, not user-facing text), all verified live against a real signed-in owner session, not just typechecked. Visual direction "Carbon Pink" approved out of 3 proposed concepts — see ADR-0016 for the full reasoning; tokens live in `tailwind.config.ts`'s `ink`/`paper`/`pink`/`confirmed`/`attention`/`vertical.*`. ADR-0016 originally namespaced these separately from the admin panel's own `brand`/`status` tokens so admin stayed functional-only per the "what NOT to build" list — **reversed 2026-08-30 per ADR-0021**: the admin panel's `brand`/`status` tokens are deleted, every `/admin/*` page now uses Carbon Pink tokens/fonts/components directly (`app/admin/layout.tsx` wires the font variables product-wide), and this was a purely visual change with zero effect on admin functionality, MFA, or access control. A small component library exists (`components/ui/`: `Button`, `Chip`, `ContactCard`, `VerticalBadge`, `AttentionBanner`, `PaymentCard`) plus a pure-component showcase at `/design-preview`. Owner authentication is built (ADR-0017: admin-provisioned accounts, no mandatory MFA) and the real "Today" screen (`app/app/(protected)/today/`) reads via `createRlsClient()`, showing the signed-in owner's own contacts/pipeline-stage/`owner_attention_queue` state. Pipeline-stage chips render with a neutral tone universally for now: there's no data-driven way yet to know which stages count as a success vs. a cancellation vs. in-progress (verified this matters against real seeded data — baker's "Cancelled" stage has a *higher* `sort_order` than "Completed"; re-investigated during the ADR-0027 simplicity pass and confirmed no clean display-only fix exists — see `docs/decisions-register.md`). The "Mark as handled" (renamed from "Review" per ADR-0027) and "Send Reminder" actions are real mutations, not presentational (ADR-0019) — Mark as handled resolves `owner_attention_queue` without moving pipeline stage; Send Reminder calls the real Phase 2 engine synchronously, capped at one per contact per day. Settings (`app/app/(protected)/settings/`) is deliberately scoped to real `businesses` profile fields only (name/phone/email/timezone/preferred_language) — CLAUDE.md's `business_settings` table documents several other "owner-editable" keys (reminder timing, instant-ack, digest frequency), but none of them have any consuming code in the shared engine yet (only `trial_grace_period_days` is ever read, and that one is billing policy, not owner-editable). Building UI for the others would mean toggles with no real effect; tracked as an open item in `docs/decisions-register.md` rather than guessed at. Dev-only fixture data for previewing against (5 businesses, one per vertical, varied attention-queue states, all connected on both channels — see ADR-0023) lives in `scripts/seed-dev-preview-data.mjs`, deliberately separate from `supabase/seed.sql` — note its one real gotcha: re-running it cascade-deletes any owner account's `business_memberships` row created against its fixture businesses (see the script's own header comment). **Added 2026-08-30 (ADR-0025)**: password reset for both surfaces (`/app/forgot-password`+`/app/reset-password`, `/admin/forgot-password`+`/admin/reset-password`, both anti-enumeration-safe and rate-limited — see `supabase/config.toml`'s `additional_redirect_urls` for the required Supabase-side allow-list entries); a shared `components/ui/AuthPageShell.tsx` for every unauthenticated auth page (fixed a real off-center-layout bug the project owner found testing directly, and added the missing logo to both login pages); Contacts List substring search over name/phone/handle (`ContactsList`'s `searchText` field, filtered client-side alongside the existing stage chips — not the full-text/universal search excluded below); a "Marked as paid." confirmation on the Payments screen; and a minimal shared loading spinner (`components/ui/PageLoading.tsx`) for both surfaces. **Added 2026-09-02 (ADR-0027, following an explicit non-technical-user simplicity review)**: "Contacts" renamed to "Customers" throughout owner-facing text (nav/headings/back-links/empty states); Contact Detail's "Pipeline Stage" heading renamed to "Status"; its "Order Details" section now hides entirely when a business has no `vertical_field_definitions` configured, instead of showing a technical empty-state sentence (in practice this section will not show anything for a real, non-fixture contact today regardless, since nothing anywhere — owner app or the message-processing engine — writes to `order_field_values` yet; investigated and reported, not built, per the project owner's explicit "confirm, don't build" instruction); Settings' Timezone and Preferred Language free-text inputs replaced with `<select>` dropdowns (India/IST is genuinely the only timezone in current use; English/Hindi are the only languages with real matching content per ADR-0007) that preserve any existing stored value even if it isn't one of the curated options; a one-line clarification that Settings' Phone/Email are the business's own profile fields, not the WhatsApp/Instagram number. Two items from that same review were investigated and explicitly **not** built pending the project owner's approval: stage-chip color-coding by outcome (needs a small additive schema column, tracked in `docs/decisions-register.md`) and per-contact manual takeover (full scope proposed in [ADR-0028](docs/architecture/decisions/0028-per-contact-manual-takeover-scope.md), awaiting sign-off).
...
4. **Launch acceptance (moved ahead of real provider integration, ADR-0020, 2026-08-29)** — **complete as of 2026-08-30** (ADR-0023). All 10 vertical×channel combos + a cross-vertical regression check + a multi-channel no-auto-merge check + opt-out, run through the actual `POST /api/webhooks/{whatsapp,instagram}` routes (not fixture inserts) via `scripts/launch-acceptance-check.mjs` — 14/14 passed, verified against real Postgres state and then cross-checked live in the owner app. Found and fixed two real gaps that predated this pass: no business anywhere had a `business_channel_connections` row (the webhook route's very first lookup depends on it — every previous test used hand-seeded data that never touched this table), and `contacts.name` was never populated by any channel (a `displayName` field existed on the WhatsApp mock payload type and was silently never read). `scripts/seed-dev-preview-data.mjs` now covers all 5 verticals (added Tutor/Gift businesses) and seeds channel connections for all of them.
