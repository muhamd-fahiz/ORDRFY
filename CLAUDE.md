...
**Marketing site (added to the plan 2026-08-29, not tied to a fixed phase number)** — currently just the unmodified `create-next-app` placeholder at `app/(marketing)/`. Decided: doesn't need to wait until the whole product is finished, but does need the owner app to be functionally complete enough to describe accurately — so it's sequenced **after the core owner-app screens are done, before Phase 6 (real provider integration) starts**, somewhere in the Phase 2.5→4→5 range rather than pinned to one of them. Visual direction will be designed separately (Claude Design), brought back, and integrated into the existing `(marketing)/` route group using the Carbon Pink design tokens (ADR-0016) — CLAUDE.md's own "more bold/aspirational freedom on the marketing site" framing from the original design-direction request still applies once that direction exists.
