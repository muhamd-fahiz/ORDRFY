...
| Open | Admin visibility into each business's *subscription payment to Ordrfy* (not their customers' own payments, which is the unrelated `payments` table and was explicitly ruled out for this) — genuinely no schema exists yet for what a business has paid Ordrfy, when, or how much they owe. `/admin/subscriptions` (ADR-0033) shows real status/filters only, no amount. Recommended smallest option: a manually-editable per-business monthly amount stored in the already-existing `business_settings` table (no new schema) rather than a full invoice-history table — needs the project owner's choice between that and the fuller option before either is built. | Admin panel shows no subscription-billing amount today, by omission, not by a UI choice | 2026-09-02 |
