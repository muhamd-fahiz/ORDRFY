...
| Open | Real subscription invoice-history for Ordrfy's own billing (`subscription_invoices`-style table: one row per billing period, amount/status/due_date/paid_at, giving a real timeline instead of one static number). Explicitly deferred by the project owner — build the simple manual-amount version now (done, ADR-0033), the real thing once actual pricing and billing collection exist (around Phase 6's Razorpay integration). | Nothing technical; `/admin/subscriptions` uses a manually-set amount in the meantime | 2026-09-02 |
...
- **Admin visibility into each business's subscription payment to Ordrfy** (opened 2026-09-02, resolved same day): build the simple version now, the real invoice-history version later. Simple version shipped as ADR-0033 (`/admin/subscriptions`, manually-editable per-business amount via `business_settings`, no new schema). The "real" half is re-opened above as its own tracked item, not forgotten.
